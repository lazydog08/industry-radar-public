...
function calculateRadar(input: {
  ...
  const trend = trendScore(input.signals, sourceCount, input.tags);
  ...
  const radar_section = sectionForScore(radar_score, video_potential, freshness_label, input.tags);
...
function relevanceScore(category: IndustryCategory, tags: string[], entities: EntityHit[], lower: string): number {
  ...
  return clampPart(score, 22);
}

function trendScore(signals: RadarSignal[], sourceCount: number, tags: string[]): number {
  const maxHeat = Math.max(0, ...signals.map((signal) => Number(signal.heatScore || 0)));
  const heat = Math.min(8, Math.log10(maxHeat + 10) * 2.4);
  const sourceSpread = Math.min(6, Math.max(0, sourceCount - 1) * 2.2);
  const crossSource = unique(signals.map((signal) => signal.source)).length >= 2 ? 3 : 0;
  const developingTags = tags.some((tag) => ["争议", "平台规则", "发布会", "系统更新", "智驾"].includes(tag)) ? 3 : 0;
  return clampPart(heat + sourceSpread + crossSource + developingTags, 20);
}
...
function changeScore(tags: string[], lower: string): number {
  ...
  for (const tag of tags) {
    if (["平台规则", "发布会", "系统更新", "智驾", "AI手机", "争议"].includes(tag)) score += 3;
  }
  const changeTerms = ["规则", "调整", "发布", "更新", "推送", "定档", "召回", "涨价", "降价", "开售", "开放", "关闭", "限制", "补贴"];
  score += changeTerms.filter((term) => lower.includes(term)).length * 1.5;
  return clampPart(score, 16);
}
...
function sectionForScore(score: number, videoPotentialScore: number, freshness: FreshnessLabel, tags: string[]): RadarSection {
  if (freshness === "stale" || freshness === "unknown") return videoPotentialScore >= 4 ? "video_ready" : "background";
  if (score >= 75) return "must_read";
  if (videoPotentialScore >= 4) return "video_ready";
...
function pushReason(input: {
  ...
  if (parts.trend >= 14) reasons.push("趋势正在扩散，值得提前观察");
  if (parts.change >= 11) reasons.push("包含明确变化信号");
  if (input.freshness_label === "new") reasons.push("近期新出现");
  if (input.video_potential >= 4) reasons.push("具备视频选题潜力");
