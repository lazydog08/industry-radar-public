...
## Decision 028
- 时间：2026-05-26 17:11:48 CST
- 决策：前端回归测试先使用 Node 内置 `node --test` 覆盖可抽离的纯逻辑，不为了一个小交互引入新的浏览器测试依赖。
- 原因：当前仓库没有测试框架或 Playwright 依赖；高级筛选摘要可以抽成无 DOM 的 ESM 模块，适合用轻量测试先建立无人值守回归入口。
- 影响：`pnpm test` 目前覆盖筛选摘要逻辑；页面级交互仍通过本地浏览器验证。后续若要覆盖静态搜索、时间线和反馈禁用，可继续抽离纯逻辑或再评估是否引入浏览器测试框架。

## Decision 029
- 时间：2026-05-26 18:47:04 CST
- 决策：整体前端采用用户选定的 E 风格「报纸头版 / 情报日报」，以 masthead、头版头条、简报栏、编辑精选三栏和下方工作台组织首页。
- 原因：用户明确反馈 E 风格最符合预期；该方向能把“今天先看什么、为什么重要、哪些能拍、哪些要补证据”放到首屏，不再像后台管理面板。
- 影响：`src/web/index.html`、`src/web/app.js`、`src/web/styles.css` 成为新的首页结构基准；搜索/高级筛选属于头版简报栏，分区列表、知识卡、报告归档和时间线属于下方工作台。
